# catches split script ----------------------------------------------------

## a script `sourced` by the .Rmd to run the catch split analysis.
...
### (2) calculating total numbers caught in upper codend
### (3) calculating total numbers caught in either codend
### (4) calculating log of upper:lower codend RF ratios - see O"Neil et al 2022 - doi:10.1016/j.fishres.2022.106297
### (5) add the length and haul specific overdispersion grouping
len_sub_dt <- len_all_dt[which(SPECIES_NAME %in% names(spl_sp_cd)), # (1)
                         .(UPPER_N = sum(N.AT.LENGTH * as.numeric(CODEND == "Upper")), # (2)
                           TOTAL_N = sum(N.AT.LENGTH), # (3)
                           LN_RF_RATIO = log(Reduce("/", Q)), # (4)
                           EPS_H_L = factor(sum(N.AT.LENGTH))),  # (5) 
                         by = .(SPECIES_NAME,
                                HAUL,
                                LENGTH.CM,
                                CONDITION,
...
# remove rows where both UPPER_N and TOTAL_N are 0
len_sub_dt <- len_sub_dt[-which(UPPER_N == 0 & TOTAL_N == 0), ]


# check: are upper codend catches sufficient for analysis? ----------------

## set inclusion limit: % hauls that must catch > 0 fish of any size
incl_limit <- 0.6

## calculate mean catches per haul per species
mean_catches_haul_sps <- len_sub_dt[, 
                                    .(mean_UPPER_N = mean(UPPER_N)), 
                                    by = .(SPECIES_NAME, HAUL)]

## calculate test statistic
incl_test <- mean_catches_haul_sps[, 
                                   sum(mean_UPPER_N == 0) / .N,
                                   by = SPECIES_NAME]
...
if (length(test_res) > 0) {
  if (show_msgs) {
    msg <- paste0("\tMessage: omitting ", test_res, " from catchSplit analysis because there are too few catches in the upper codend.\n")
    message(msg)
  }
...
len_sub_dt$EPS_H_L <- factor(len_sub_dt$EPS_H_L)
...
for (i in names(m0_lst)) {
  ...
  ## fit saturated model; m = 1 to simplify smooth
  if(length(unique(d_lst[[i]]$LN_RF_RATIO > 5))) { # added in revision to fix Northern Squid offset
    m0 <- bam(
    formula = cbind(UPPER_N, (TOTAL_N - UPPER_N)) ~ # response variable
      ...
      s(LENGTH.CM, HAUL, bs = "fs", k = ifelse(nlevels(d$EPS_H_L) > 5, 5, 4), m = 1) + # non-linear random effect
      s(EPS_H_L, bs = "re") + # overdispersion parameter
      offset(LN_RF_RATIO), # sub-sampling fraction ratio
...
for (i in names(ms_lst)) {
  ...
  m <- update(m0, 
              ...
              - s(LENGTH.CM, by = CONDITION, m = 1)
              - s(LENGTH.CM, by = LC, m = 1)
              - s(LENGTH.CM, HAUL, bs = "fs", k = ifelse(nlevels(d$EPS_H_L) > 5, 5, 4), m = 1))
  s_lst[["no length"]] <- m
  rm(m)
  gc()
  ...
  nm <- paste0("prop_upper_mod_simp_aicctab_", gsub(" ", "_", i), ".csv")
  write.csv(aicc_tab, file = here("results", nm), row.names = FALSE)
  nm <- paste0("prop_upper_mod_simp_bictab_", gsub(" ", "_", i), ".csv")
  ...
  p <- ggplot() +
    ...
    guides(shape = guide_legend(override.aes = list(shape = c(16, NA),
           ...
           linetype = guide_legend(override.aes = list(shape = c(16, NA),
                                                       linetype = c("blank", "solid")))) +
    labs(x = "Length (cm)", y = "Proportion in upper codend", linetype = "Type", shape = "Type") +
    sgg()
  
  ## save plot
  if (make_plots) {
    nm <- paste0("prop_upper_", i, "_bestmod.pdf")
    nm <- gsub("\\.pdf", "_R1\\.pdf", nm) # revision
    cairo_pdf(here("plots", nm), 
              width = 10, height = 7)
    print(p)
    dev.off()
    nm <- paste0("prop_upper_", i, "_bestmod.jpg")
...
## make plot
prop_upper_emmeans_p <- ggplot(best_inf, 
                               aes(x = `Time of day`, y = estimate, colour = Lights)) + 
  ...
  facet_wrap(~ Species) +
  xlab("Time of day") +
  ylab("Log odds ratio of effect\n") +
  scale_color_manual(values = c("#F8766D", "#00BFC4", "gray50"), drop = FALSE) +
  scale_x_discrete(drop = FALSE) +
  sgg()

## save plot
if (make_plots) {
  nm <- "prop_upper_allspecies_emmeans.pdf"
  nm <- gsub("\\.pdf", "_R1\\.pdf", nm) # revision
  cairo_pdf(here("plots", nm), 
            width = 10, height = 7)
  print(prop_upper_emmeans_p)
  dev.off()
  nm <- "prop_upper_allspecies_emmeans.jpg"
  nm <- gsub("\\.jpg", "_R1\\.jpg", nm) # revision
  jpeg(here("plots", nm), 
...
if (save_out) {
  ...
  ## results name
  save_nm <- paste0("prop_upper_results_", comp_stat, "_R1.RData")
